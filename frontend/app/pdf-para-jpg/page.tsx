...
export default function PDFParaJPG() {
  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await apiPost('/pdf-to-jpg', formData);

      if (!response.ok) throw new Error('Erro ao converter PDF para JPG');

      const pages = parseInt(response.headers.get('X-Pages-Converted') || '0');
      setPagesConverted(pages);
      setSuccess(true);

      await saveResponseFiles(response, `converted_${file.name.replace('.pdf', '')}.jpg`);
    } catch (err) {
  ...
  return (
    <>
      ...
      {loading && <LoadingSpinner message="Convertendo PDF para imagens…" />}
      ...
      <div className="page-root">
        <div className="page-inner">
          <nav className="breadcrumb">
            <a href="/">Início</a>
            <span>/</span>
            <span className="breadcrumb-current">PDF para JPG</span>
          </nav>
          ...
          <div className="page-header">
            ...
            <h1 className="page-title">PDF para JPG</h1>
            <p className="page-subtitle">Converta cada página do seu PDF em imagens JPG de alta qualidade.</p>
          </div>

          <div className="card">
            {/* Uploader igual ao do Mesclar PDFs */}
            ...
            {file && !success && (
              <button
                ...
                disabled={loading}
              >
                {loading ? 'Convertendo…' : '🖼️ Converter para JPG'}
              </button>
            )}
            ...
            {success && (
              <div className="success-box">
                ...
                <p className="success-p">
                  {pagesConverted} {pagesConverted === 1 ? 'página convertida' : 'páginas convertidas'} para JPG.
                  <br />
          ...
          <div className="features-mini">
            {[
              { e: '🎨', t: 'Alta resolução', d: 'Imagens nítidas e claras' },
