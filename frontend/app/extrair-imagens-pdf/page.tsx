...
export default function ExtrairImagensPDF() {
  ...
  const handleExtract = async () => {
    ...
    try {
      const response = await apiPost('/extract-images', formData);

      if (!response.ok) {
        const errBody = await response.text();
        throw new Error(errBody || 'Erro ao extrair imagens do PDF');
      }

      const count = parseInt(response.headers.get('X-Images-Extracted') || '0');
      setImagesExtracted(count);
      setSuccess(true);

      await saveResponseFiles(response, `imagens_${file.name.replace('.pdf', '')}`);
    } catch (err) {
      setError('Erro ao extrair as imagens. Tente novamente.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      {loading && <LoadingSpinner message="Extraindo imagens do PDF…" />}
      ...
      <div className="page-root">
        <div className="page-inner">
          <nav className="breadcrumb">
            <a href="/">Início</a>
            <span>/</span>
            <span className="breadcrumb-current">Extrair Imagens do PDF</span>
          </nav>
          ...
          <div className="page-header">
            ...
            <h1 className="page-title">Extrair Imagens do PDF</h1>
            <p className="page-subtitle">Extraia todas as imagens embutidas no seu PDF como arquivos separados.</p>
          </div>
          ...
          <div className="card">
            ...
            {file && !success && (
              <button
                ...
                disabled={loading}
              >
                {loading ? 'Extraindo…' : '🖼️ Extrair imagens'}
              </button>
            )}
            ...
            {success && (
              <div className="success-box">
                ...
                <p className="success-p">
                  {imagesExtracted} {imagesExtracted === 1 ? 'imagem extraída' : 'imagens extraídas'} do PDF.
                  <br />
          ...
          <div className="features-mini">
            {[
              { e: '🖼️', t: 'Imagens originais', d: 'Qualidade original sem perdas' },
              { e: '📦', t: 'Sem ZIP', d: 'Todas as imagens separadas' },
