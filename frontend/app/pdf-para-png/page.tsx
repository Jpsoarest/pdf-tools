...
export default function PDFParaPNG() {
  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await apiPost('/pdf-to-png', formData);

      if (!response.ok) throw new Error('Erro ao converter PDF para PNG');

      const pages = parseInt(response.headers.get('X-Pages-Converted') || '0');
      setPagesConverted(pages);
      setSuccess(true);

      await saveResponseFiles(response, `convertido_${file.name.replace('.pdf', '')}.png`);
    } catch (err) {
      setError('Erro ao converter o PDF. Tente novamente.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      {loading && <LoadingSpinner message="Convertendo PDF para imagens PNG…" />}
      ...
      <div className="page-root">
        <div className="page-inner">
          <nav className="breadcrumb">
            <a href="/">Início</a>
            <span>/</span>
            <span className="breadcrumb-current">PDF para PNG</span>
          </nav>
          ...
          <div className="page-header">
            ...
            <h1 className="page-title">PDF para PNG</h1>
            <p className="page-subtitle">Converta cada página do seu PDF em imagens PNG de alta qualidade com fundo transparente.</p>
          </div>
          ...
          <div className="card">
            ...
            {file && !success && (
              <button
                ...
                disabled={loading}
              >
                {loading ? 'Convertendo…' : '🖼️ Converter para PNG'}
              </button>
            )}
            ...
            {success && (
              <div className="success-box">
                ...
                <p className="success-p">
                  {pagesConverted} {pagesConverted === 1 ? 'página convertida' : 'páginas convertidas'} para PNG.
                  <br />
                  As imagens foram salvas na pasta de saída.
                </p>
          ...
          <div className="features-mini">
            {[
              { e: '🫧', t: 'Transparência', d: 'Imagens com fundo transparente' },
              { e: '⚡', t: 'Conversão rápida', d: 'Processamento em segundos' },
              { e: '🔒', t: 'Privacidade total', d: 'Arquivos deletados após uso' },
            ].map((x) => (
