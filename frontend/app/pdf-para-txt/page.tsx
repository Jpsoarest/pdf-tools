...
export default function PdfParaTxt() {
  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await apiPost('/pdf-to-txt', formData);

      if (!response.ok) throw new Error('Erro ao extrair texto do PDF');

      const pages = parseInt(response.headers.get('X-Total-Pages') || '0');
      const chars = parseInt(response.headers.get('X-Chars-Extracted') || '0');
      setTotalPages(pages);
      setCharsExtracted(chars);
      setSuccess(true);

      await saveResponseFiles(response, `${file.name.replace('.pdf', '')}.txt`);
    } catch {
      setError('Erro ao extrair o texto do PDF. Tente novamente.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      {loading && <LoadingSpinner message="Extraindo texto do PDF…" />}
      ...
      <div className="page-root">
        <div className="page-inner">
          <nav className="breadcrumb">
            <a href="/">Início</a>
            <span>/</span>
            <span className="breadcrumb-current">PDF para TXT</span>
          </nav>
          ...
          <div className="page-header">
            ...
            <h1 className="page-title">PDF para TXT</h1>
            <p className="page-subtitle">Extraia todo o texto do seu PDF em um arquivo de texto puro.</p>
          </div>
          ...
          <div className="card">
            ...
            {file && !success && (
              <button
                ...
                disabled={loading}
              >
                {loading ? 'Convertendo…' : '📝 Extrair texto'}
              </button>
            )}
            ...
            {success && (
              <div className="success-box">
                ...
                <p className="success-p">
                  O texto foi extraído com sucesso do PDF.
                  <br />
                  O arquivo TXT foi salvo na pasta de saída.
                </p>
                <div className="stats-row">
                  ...
                  <div className="stat-card">
                    ...
                    <div className="stat-value">{charsExtracted.toLocaleString()}</div>
                  </div>
                </div>
          ...
          <div className="features-mini">
            {[
              { e: '📝', t: 'Texto puro', d: 'Saída limpa e sem formatação' },
              { e: '📑', t: 'Todas as páginas', d: 'Extrai texto de todo o documento' },
