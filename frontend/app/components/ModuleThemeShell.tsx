...
export default function ModuleThemeShell({ children }: Readonly<{ children: ReactNode }>) {
  const isOficioMode = useOficioMode();

  return (
    <main className={`main-content${isOficioMode ? ' oficio4-theme' : ''}`}>
      <style>{`
        .oficio4-theme {
          --bg-primary: #f7f3ee;
          --bg-secondary: #fffaf4;
        ...
        .oficio4-theme .toolshell {
          background: linear-gradient(180deg, rgba(16,38,61,.06), transparent 320px), #f7f3ee;
        }
