...
export default function ComprimirImagem() {
  ...
  const handleCompress = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Erro ao comprimir imagem');
      ...
      if (reduction > 0) {
        ...
        let ext = file.name.split('.').pop() || 'img';
        if (targetFormat !== 'original') ext = targetFormat;
        const baseName = file.name.replace(/\.[^.]+$/, '');
        await downloadBlob(blob, `comprimida_${baseName}.${ext}`);
      }
    } catch (err) {
    ...
    <>
      <style>{`
        ...
        @media (max-width: 480px) {
          ...
          .stat-card-big { grid-column: auto; }
        }
      `}</style>

      {loading && <LoadingSpinner message="Comprimindo sua imagem..." />}
      ...
      <div className="page-root">
        <div className="page-inner">
          <nav className="breadcrumb">
            <a href="/">Início</a>
            <span>/</span>
            <span className="breadcrumb-current">Comprimir Imagem</span>
          </nav>
          ...
          <div className="page-header">
            ...
            <h1 className="page-title">Comprimir Imagem</h1>
            <p className="page-subtitle">Reduza o tamanho de imagens sem perder qualidade visível.</p>
          </div>
          ...
          <div className="card">
            ...
            {file && !stats && (
              <>
                {previewUrl && (
                  <div className="preview-section">
                    ...
                    <div className="preview-overlay">
                      ...
                      <span className="preview-badge">{formatBytes(file.size)}</span>
                    </div>
                  </div>
                )}
                ...
                <button
                  ...
                  disabled={loading}
                >
                  {loading ? 'Comprimindo…' : '🗜️ Comprimir Imagem'}
                </button>
              </>
            )}
            ...
            {!file && (
              <div className="info-box">
                ...
                <p>A compressão é inteligente e ajustável. Nenhuma imagem é armazenada em nossos servidores.</p>
              </div>
            )}
            ...
            {stats && stats.reduction <= 0 && (
              <div className="optimized-box">
                <h3>Imagem já otimizada</h3>
                <p>Esta imagem já está no menor tamanho possível para a qualidade selecionada ({formatBytes(stats.originalSize)}).</p>
            ...
            {stats && stats.reduction > 0 && (
              <div className="success-box">
                <div className="success-header">
                  ...
                  <h3>Imagem comprimida com sucesso!</h3>
                </div>
                <p className="success-p">O download foi iniciado automaticamente.</p>
                <div className="stats-grid">
                  <div className="stat-card">
                    <div className="label">Original</div>
                    <div className="value">{formatBytes(stats.originalSize)}</div>
                  </div>
                  <div className="stat-card">
                    <div className="label">Comprimido</div>
                    <div className="value">{formatBytes(stats.compressedSize)}</div>
                  </div>
                  <div className="stat-card-big">
                    <div className="label">Você economizou</div>
                    <div className="value">{stats.reduction.toFixed(1)}%</div>
                    <div className="sub">{formatBytes(stats.originalSize - stats.compressedSize)} liberados</div>
                  </div>
                </div>
                <button className="btn-outline" onClick={removeFile}>
          ...
          <div className="trust-grid">
            {[
              { e: '🧠', t: 'Compressão Inteligente', d: 'Algoritmos que preservam os detalhes' },
              { e: '🎚️', t: 'Qualidade Ajustável', d: 'Controle deslizante de 10 a 100' },
              { e: '👁️', t: 'Sem Perda Visível', d: 'Redução sem comprometer a aparência' },
            ].map((x) => (
